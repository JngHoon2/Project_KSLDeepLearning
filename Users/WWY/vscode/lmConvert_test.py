...
detector = htm.handDetector(detectionCon=0.75)  # 핸드 디텍터 생성


def convert(gesture_folder, target_folder):
    # rootPath 변수에 현재 디렉토리의 위치를 저장합니다.
    rootPath = os.getcwd()
    # majorData 변수에 target_foler의 정규화된 절대 경로명을 저장합니다.
    majorData = os.path.abspath(target_folder)

    # majorData 변수에 저장된 폴더가 존재하는 지 확인합니다. exists(x)-> x가 존재하면 True 반환
    if not exists(majorData):
        os.makedirs(majorData)                  # majorData 라는 이름의 폴더를 생성합니다.

    # gesture_folder 변수에 함수에서 받은 매개 변수 gesture_folder의 정규절대경로명을 저장합니다.
    gesture_folder = os.path.abspath(gesture_folder)

    # 현재 디렉토리의 경로를 gesture_folder 로 변경합니다.
    os.chdir(gesture_folder)
    # gestures 라는 변수에 현재 디렉토리의 리스트를 저장합니다.
    ...
    # gesture_path 변수에 gesture_folder와 gesture 의 경로명을 합친 값을 저장합니다.
    gesture_path = os.path.join(gesture_folder, gesture)
    # 현재 디렉토리의 경로를 gesture_path 로 변경합니다.
    os.chdir(gesture_path)

    # gesture_frames_path 변수에 majorData와 gesture의 경로명을 합친 값을 저장합니다.
    gesture_frames_path = os.path.join(majorData, gesture)
    # gesture_frames_path 폴더가 존재하는 지 확인합니다.
    if not os.path.exists(gesture_frames_path):
        # gesture_frames_path 라는 이름의 폴더를 생성합니다.
        os.makedirs(gesture_frames_path)

    # videos 라는 변수에 현재 디렉토리의 리스트를 저장합니다.
    videos = os.listdir(os.getcwd())
    # videos 리스트의 인덱스(video)가 존재하면 video를 반환하여 videos리스트에 저장합니다
    ...
    for video in tqdm(videos, unit='videos', ascii=True):
        ...
        # 인덱스의 경로명을 name 변수에 저장합니다.
        name = os.path.abspath(video)
        # capturing input video / name 비디오를 이용합니다.
        cap = cv2.VideoCapture(name)
        # 사용한 비디오의 프레임 수를 저장합니다.
        frameCount = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
        # lastFrame 변수를 NULL 상태로 정의합니다.
        lastFrame = None

        # 현재 경로를 gesture_frames_path 로 변경합니다.
        os.chdir(gesture_frames_path)
        # count 변수가 0인 상태로 정의됩니다.
        count = 0

        # assumption only first 200 frames are important / 비디오에서 처음 200 프레임만 따옵니다.
        while count < 200:
            # extract frame / 파일을 읽습니다. 성공시 수신한 바이트 수를 리턴하며 실패시 -1을 반환합니다.
            ret, frame = cap.read()
